...
- **FR-008**: System MUST screen uploaded files before making them available to users.
...
- **Document**: A work-related file and its business metadata, including title, description, category, tags, upload details, accessible file type information, and optional relationships to a project or task.
- **Document Share**: A record that grants a user or team access to a document that is not otherwise visible through direct ownership or project membership.
- **Document Activity Record**: An audit entry describing a document event such as upload, preview, download, deletion, or sharing, along with who performed it and when.
